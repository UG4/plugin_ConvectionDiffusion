...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
elem_loop_finish_crfv()
{}

template<typename TDomain>
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
elem_prepare_crfv(TElem* elem, const LocalVector& u)
{
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ass_JA_elem_crfv(LocalMatrix& J, const LocalVector& u)
{
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ass_JM_elem_crfv(LocalMatrix& J, const LocalVector& u)
{
// 	get finite volume geometry
	const static TFVGeom& geo = Provider<TFVGeom>::get();
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ass_dA_elem_crfv(LocalVector& d, const LocalVector& u)
{
// 	get finite volume geometry
	const static TFVGeom& geo = Provider<TFVGeom>::get();
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ass_dM_elem_crfv(LocalVector& d, const LocalVector& u)
{
// 	get finite volume geometry
	const static TFVGeom& geo = Provider<TFVGeom>::get();
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ass_rhs_elem_crfv(LocalVector& d)
{
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_velocity_crfv(const LocalVector& u,
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_diffusion_crfv(const LocalVector& u,
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_reaction_rate_crfv(const LocalVector& u,
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_reaction_crfv(const LocalVector& u,
                     std::vector<std::vector<number> > vvvLinDef[],
                     const size_t nip)
{
//  get finite volume geometry
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_source_crfv(const LocalVector& u,
                   std::vector<std::vector<number> > vvvLinDef[],
                   const size_t nip)
{
//  get finite volume geometry
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_mass_scale_crfv(const LocalVector& u,
                       std::vector<std::vector<number> > vvvLinDef[],
                       const size_t nip)
{
//  get finite volume geometry
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
lin_def_mass_crfv(const LocalVector& u,
                       std::vector<std::vector<number> > vvvLinDef[],
                       const size_t nip)
{
//  get finite volume geometry
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ex_value_crfv(const LocalVector& u,
...
template <typename TElem, typename TFVGeom>
void ConvectionDiffusion<TDomain>::
ex_grad_crfv(const LocalVector& u,
                          const MathVector<dim> vGlobIP[],
                          const MathVector<TFVGeom::dim> vLocIP[],
                          const size_t nip,
...
register_all_crfv_funcs(bool bHang)
{
//	switch assemble functions
	if(!bHang)
	{
		register_crfv_func<Edge, CRFVGeometry<Edge, dim> >();
	}
	else
	{
...
//	switch assemble functions
	if(!bHang)
	{
		register_crfv_func<Triangle, CRFVGeometry<Triangle, dim> >();
		register_crfv_func<Quadrilateral, CRFVGeometry<Quadrilateral, dim> >();
	}
	else
	{
		UG_THROW("ConvectionDiffusion"
...
//	switch assemble functions
	if(!bHang)
	{
		register_crfv_func<Tetrahedron, CRFVGeometry<Tetrahedron, dim> >();
		register_crfv_func<Prism, CRFVGeometry<Prism, dim> >();
		register_crfv_func<Pyramid, CRFVGeometry<Pyramid, dim> >();
		register_crfv_func<Hexahedron, CRFVGeometry<Hexahedron, dim> >();
	}
	else
	{
		UG_THROW("ConvectionDiffusion"
...
register_crfv_func()
{
	...
	this->set_prep_elem_loop_fct(id, &T::template elem_loop_prepare_crfv<TElem, TFVGeom>);
	this->set_prep_elem_fct(	 id, &T::template elem_prepare_crfv<TElem, TFVGeom>);
	this->set_fsh_elem_loop_fct( id, &T::template elem_loop_finish_crfv<TElem, TFVGeom>);
	this->set_ass_JA_elem_fct(   id, &T::template ass_JA_elem_crfv<TElem, TFVGeom>);
	this->set_ass_JM_elem_fct(   id, &T::template ass_JM_elem_crfv<TElem, TFVGeom>);
	this->set_ass_dA_elem_fct(   id, &T::template ass_dA_elem_crfv<TElem, TFVGeom>);
	this->set_ass_dM_elem_fct(   id, &T::template ass_dM_elem_crfv<TElem, TFVGeom>);
	this->set_ass_rhs_elem_fct(  id, &T::template ass_rhs_elem_crfv<TElem, TFVGeom>);

//	set computation of linearized defect w.r.t velocity
	m_imVelocity. set_fct(id, this, &T::template lin_def_velocity_crfv<TElem, TFVGeom>);
	m_imDiffusion.set_fct(id, this, &T::template lin_def_diffusion_crfv<TElem, TFVGeom>);
	m_imReactionRate. set_fct(id, this, &T::template lin_def_reaction_rate_crfv<TElem, TFVGeom>);
	m_imReaction. set_fct(id, this, &T::template lin_def_reaction_crfv<TElem, TFVGeom>);
	m_imSource.	  set_fct(id, this, &T::template lin_def_source_crfv<TElem, TFVGeom>);
	m_imMassScale.set_fct(id, this, &T::template lin_def_mass_scale_crfv<TElem, TFVGeom>);
	m_imMass.	set_fct(id, this, &T::template lin_def_mass_crfv<TElem, TFVGeom>);

//	exports
	m_exValue->	   template set_fct<T,refDim>(id, this, &T::template ex_value_crfv<TElem, TFVGeom>);
	m_exGrad->template set_fct<T,refDim>(id, this, &T::template ex_grad_crfv<TElem, TFVGeom>);
}
