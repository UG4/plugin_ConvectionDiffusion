...
add_jac_A_elem(LocalMatrix& J, const LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[])
{
	...
	TFVGeom& geo = GeomProvider<TFVGeom>::get(m_LFEID,1);
	bool bElementIsCut = geo.get_element_modus();
	...
	if ( !bElementIsCut )
	{
		this->template add_jac_A_elem_local<TElem,TFVGeom> (geo, J, u, elem, vCornerCoords, bElementIsCut);
		return;
	}
...
// reset data:
	...
	this->template add_jac_A_elem_local<TElem,TFVGeom> (geo, locJ_tri, u, elem, vCornerCoords, bElementIsCut);
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusionFV1<TDomain>::
add_jac_A_elem_local(TFVGeom& geo, LocalMatrix& J, const LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsCut)
{
	...
	for(size_t ip = 0; ip < geo.num_scv(); ++ip)
		areaElem += geo.scv(ip).volume();

	number areaScale = 1.0;

	if ( bElementIsCut )
	{
		areaScale = geo.Area()/areaElem;

		UG_LOG("check area: areaElem = " << areaElem << " geo.AreaOrig() = " <<  geo.AreaOrig() << "\n");

		if ( fabs(areaScale - geo.AreaScale()) > 0.00001 )
			UG_THROW("error for area: areaScale = " << areaScale << " geo.AreaScale() = " <<  geo.AreaScale() << "\n");
	}

	//areaScale = 1.0;

	const number kappa_1 = areaScale;
	const number alpha_1 = 1.0;
	const number scaleFactor = alpha_1 * kappa_1;
...
//	Diffusion and Velocity Term
		...
		////////////////////////////////////////////////////
			...
			// 	loop shape functions
				...
				//	Compute flux at IP
                    ...
                    UG_LOG("sh = " << sh << " D_diff_flux = " << D_diff_flux << "\n");

					J(_C_, scvf.from(), _C_, sh) -= areaScale * D_diff_flux;
					J(_C_, scvf.to()  , _C_, sh) += areaScale * D_diff_flux;
				}
	...
	{
		//	loop trial space
		MathVector<dim> NormalToFace = geo.NormalToFace();
		...
		//	Diffusion
			if(m_imDiffusion.data_given())
				MatVecMult(Dgrad, m_imDiffusion[0], scvf.global_grad(j));
			else
				VecSet(Dgrad, 0.0);
			...
			{
			// multiply by integral along Gamma of shape fct \phi_i
				number integrand = VecDot(Dgrad, NormalToFace) * geo.IntegralGamma(i);
				UG_LOG("geo.IntegralGamma(i): " << geo.IntegralGamma(i) << "\n");

			//	SUBTRACT to local matrix
				J(_C_, i, _C_, j) -= scaleFactor * integrand; // = (grad(U), phi_i)
				J(_C_, j, _C_, i) -= scaleFactor * integrand; // = (U, grad(phi_j))
...
add_def_A_elem(LocalVector& d, const LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[])
{
	...
	int orientation = 1;
 	...
 	if ( bElementIsCut )
 	{
 		geo.print_Nitsche_Data();

 		for(size_t i = 0; i < 3; ++i)
 		{
 			UG_LOG("vAlpha(i,0): " << geo.vAlpha(i,0) << "\n");
	...
	if ( !bElementIsCut )
	{
		this->template add_def_A_elem_local<TElem,TFVGeom> (geo, d, u, elem, vCornerCoords, bElementIsCut);
		return;
	}
...
// reset data:
	...
	this->template add_def_A_elem_local<TElem,TFVGeom> (geo, locD_tri, u, elem, vCornerCoords, bElementIsCut);
...
//	compute global integration points
	...
	UG_LOG("vGlobIP" << vGlobIP[0][0] << " and " << vGlobIP[0][1] << "\n" );
    UG_LOG("\n" );
...
//	compute local integration points
    ...
    UG_LOG("vLocIP" << vLocIP[0][0] << " and " << vLocIP[0][1] << "\n" );
    UG_LOG("\n" );
...
template<typename TElem, typename TFVGeom>
void ConvectionDiffusionFV1<TDomain>::
add_def_A_elem_local(TFVGeom& geo, LocalVector& d, const LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsCut)
{
	ug::MathMatrix<dim, dim, double> diffusion = m_imDiffusion[0];
	diffusion *= 1.0;
	...
	if ( bElementIsCut )
	{
		...
		if ( fabs(areaScale - geo.AreaScale()) > 0.00001 )
			UG_LOG("error for area: areaScale = " << areaScale << " geo.AreaScale() = " <<  geo.AreaScale() << "\n");
	}
	...
	for(size_t ip = 0; ip < geo.num_scvf(); ++ip)
	{
		...
		if(m_imDiffusion.data_given())
		{
            ...
            MathVector<dim> Dgrad_debug, grad_debug;
            ...
            for(size_t sh = 0; sh < scvf.num_sh(); ++sh)
            {
                VecSet(grad_debug, 0.0);
                VecScaleAppend(grad_debug, 1.0, scvf.global_grad(sh));
                MatVecMult(Dgrad_debug, diffusion, grad_debug);
                const number diff_flux_debug = VecDot(Dgrad_debug, scvf.normal());

                UG_LOG("sh: " << sh << "diff_flux_debug = " << diff_flux_debug << "\n");
            }
            
			// 	Add to local defect
			d(_C_, scvf.from()) -= areaScale * diff_flux;
			d(_C_, scvf.to()  ) += areaScale * diff_flux;
	...
	if ( bElementIsCut )
	{
		...
		const typename TFVGeom::SCVF& scvf = geo.scvf(0);
		////////////////////////////////////////////////////
		// FIRST TERM OF BOUNDARY
		for(size_t j = 0; j < geo.num_sh(); ++j)
			VecScaleAppend(grad, u(_C_,j), scvf.global_grad(j));

		MatVecMult(Dgrad, diffusion, grad);

		number integrand1_1 = VecDot(Dgrad, NormalToFace);

		//	loop test space
		for(size_t i = 0; i < geo.num_sh(); ++i)
		{
		// multiply by integral along Gamma of shape fct \phi_i
			number integrand2_1 = geo.IntegralGamma(i);
			UG_LOG("geo.IntegralGamma(i): " << geo.IntegralGamma(i) << "\n");

		//	SUBTRACT to local defect
			d(_C_, i) -= scaleFactor * integrand1_1 * integrand2_1;

		} // end i-loop


		////////////////////////////////////////////////////
		// SECOND TERM OF BOUNDARY

		number integrand2_1 = 0.0;
		for(size_t j = 0; j < geo.num_sh(); ++j)
			integrand2_1 += u(_C_,j) * geo.IntegralGamma(j);
		...
		for(size_t i = 0; i < geo.num_sh(); ++i)
		{
			...
			MatVecMult(Dgrad, diffusion, scvf.global_grad(i));
			number integrand2_2 = VecDot(Dgrad, NormalToFace);

		//	SUBTRACT to local defect
			d(_C_, i) -= scaleFactor * integrand2_1 * integrand2_2;
			// da jump [U] = 0!
		} // end i-loop
	}
	...
	//////////////////////////////////////////////////////////////
	// Source Term - NOT added during 'add_rhs_elm()':
	// during elem_disc_assemble: rhs is NOT added anymore!!
	//////////////////////////////////////////////////////////////
	...
	if ( m_imSource.data_given() )
	{
		for ( size_t ip = 0; ip < geo.num_scv(); ++ip ) {
			...
			// Add to local rhs
			d(_C_, co) += areaScale * m_imSource[ip] * scv.volume();
			UG_LOG("scv.volume() = " << scv.volume() << "\n");
		}
	}
