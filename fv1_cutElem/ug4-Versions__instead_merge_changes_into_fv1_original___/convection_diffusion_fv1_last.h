...
class ConvectionDiffusionFV1 : public ConvectionDiffusionBase<TDomain>
{
	...
	///	assembles the local stiffness matrix using a finite volume scheme
		...
		template <typename TElem, typename TFVGeom>
		void add_jac_A_elem_local(TFVGeom& geo, LocalMatrix& J, const LocalVector& u, const LocalVector& jump, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsOutside);

		template <typename TElem, typename TFVGeom>
		void add_jac_A_elem_boundary(TFVGeom& geo, LocalMatrix& J, const LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsOutside);
	...
	///	assembles the stiffness part of the local defect
		template <typename TElem, typename TFVGeom>
		void add_def_A_elem_local(TFVGeom& geo, LocalVector& d, const LocalVector& u, const LocalVector& jump, const LocalVector& jump_grad, const LocalVector& source, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsOutside);

		template <typename TElem, typename TFVGeom>
		void add_def_A_elem_boundary(TFVGeom& geo, LocalVector& d, LocalVector& u, GridObject* elem, const MathVector<dim> vCornerCoords[], const bool bElementIsOutside);
	...
	///	type of trial space for each function used
		virtual void prepare_setting(const std::vector<LFEID>& vLfeID, bool bNonRegularGrid, bool bStaticRoid);
