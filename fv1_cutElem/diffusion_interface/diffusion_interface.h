		...
		: public IMovingInterface<TDomain, TAlgebra>
{
	...
	//////////////////////////////////////////////////////////////////////////////////
		...
		{ return m_spInterfaceHandlerLocal->set_Nitsche(bNitsche); }
 		...
 		void init(vector_type& u, SmartPtr<ApproximationSpace<TDomain> > spApproxSpace, const int baseLevel, const int topLevel, bool bScaleDoFs, bool bBndFct)
		{
		...
		// values for new DoFs are set to 0.0 by the 'resize()'-method (see vector.h):
            ...
            m_spInterfaceHandlerLocal->check_interface_data(bBndFct);
        ...
        void set_source_data(const number interfaceSourceVal) { m_spInterfaceHandlerLocal->set_source_lua(interfaceSourceVal); }
        void set_jump_data(const number interfaceJumpVal) { m_spInterfaceHandlerLocal->set_jump_lua(interfaceJumpVal); }
        void set_jump_grad_data(const MathVector<2>& interfaceJumpGradVal) { m_spInterfaceHandlerLocal->set_jump_grad_lua(interfaceJumpGradVal); }
        void set_diffusion_data(const MathVector<2>& diffusionCoeffs) { m_spInterfaceHandlerLocal->set_diffusion_coeff_lua(diffusionCoeffs); }
