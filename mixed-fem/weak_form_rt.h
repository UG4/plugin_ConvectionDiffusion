...
#ifndef __H__UG__LIB_DISC__CONVECTION_DIFFUSION__WEAK_FORMULATION_FE__
#define __H__UG__LIB_DISC__CONVECTION_DIFFUSION__WEAK_FORMULATION_FE__
...
class WeakFormulationFE : public ConvectionDiffusionBase<TDomain>
{
	private:
	///	Base class type
	...
	///	Own type
		typedef WeakFormulationFE<TDomain> this_type;
...
public:
	...
	///	computes the concentration
		template <typename TElem, typename TFEGeom>
		void ex_divergence(number vValue[],
	...
	///	computes the gradient of the concentration
		template <typename TElem, typename TFEGeom>
		void ex_field(MathVector<dim> vValue[],
		             const MathVector<dim> vGlobIP[],
		             number time, int si,
		             const LocalVector& u,
		             GridObject* elem,
